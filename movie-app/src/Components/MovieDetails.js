...
const MovieDetails = ({ selected, toggleModal, modalOpen, movies }) => {
            ...
            tags = Ratings.map((ratingObj, i) => {
                ...
                return (
                    <div className="rating-container" key={i}>
                        <img className="rating-logo" src={sourceLogo(source)} alt={`${source}`} />
    ...
    return (
        <div className={`modal-container ${modalOpen ? `show-modal` : `hide-modal`}`} onClick={toggleModal}>
            {/* Background overlay */}
            <div style={modalPosterOverlay} className="modal">
